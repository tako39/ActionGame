	...
	for (int num = 0; num < MAX_ENEMY; num++) {
		if (enemy[num] != NULL) {
			...
			if (enemy[num]->GetHitPoint() <= 0) {	//HP‚ª0ˆÈ‰º‚Ì‚Æ‚«
				...
				if (type == ENEMY_ZAKO) {			//Á‚¦‚é
					enemy[num] = NULL;
				}
				else if (type == ENEMY_TALL) {		//Zako‚ğ¶¬
					VECTOR tallPos = enemy[num]->GetPos();
					enemy[num] = new Zako();
					enemy[num]->SetPos(tallPos);
				}
				else if (type == ENEMY_BIG) {		//Tall‚ğ¶¬
					VECTOR bigPos = enemy[num]->GetPos();
					enemy[num] = new Tall();
					enemy[num]->SetPos(bigPos);
				}
				else if (type == ENEMY_BOSS) {		//Á‚¦‚é
					enemy[num] = NULL;
				}
			}
		}
